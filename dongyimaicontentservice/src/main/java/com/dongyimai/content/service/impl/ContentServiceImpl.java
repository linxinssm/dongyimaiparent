...
public class ContentServiceImpl implements ContentService {
	...
	public void update(TbContent content){
		...
		Long categoryId = contentMapper.selectByPrimaryKey(content.getId()).getCategoryId();//原来的categoryId

		//删除redis中原来该对象对应categoryId
		redisTemplate.boundHashOps("contentList").delete(categoryId);

		//2、判断要修改的content.getCategoryId() 和 原来的categoryId 是否为同一个
		if(categoryId != content.getCategoryId()){
			redisTemplate.boundHashOps("contentList").delete(content.getCategoryId());//删除要修改的categoryId
		}
    ...
    public List<TbContent> findContentList(Long categoryId) {
		...
		//2.如果contentList为空null,去mysql中取值
